...
const fetchNews = async (topic: string, force = false): Promise<NewsResponse> => {
  // Candidate endpoints to handle different proxy/container routing rules
  const base = import.meta.env.BASE_URL || '/';
  const cleanBase = base.endsWith('/') ? base : `${base}/`;
  const forceParam = force ? '&force=true' : '';
  const endpoints = [
    `${cleanBase}api/news?topic=${encodeURIComponent(topic)}${forceParam}`,
    `/api/news?topic=${encodeURIComponent(topic)}${forceParam}`,
    `/web/api/news?topic=${encodeURIComponent(topic)}${forceParam}`,
  ];
      ...
      if (json && Array.isArray(json.items) && json.items.length > 0) {
        ...
        if (validItems.length > 0) {
          return {
            ...
            dailyEdition: typeof json.dailyEdition === 'string' ? json.dailyEdition : undefined,
            nextDailyUpdate: typeof json.nextDailyUpdate === 'string' ? json.nextDailyUpdate : undefined,
            updateFrequency: typeof json.updateFrequency === 'string' ? json.updateFrequency : 'Daily (Refreshed once a day)',
          };
        }
      }
    } catch (err) {
  ...
  return (
    <section id="news" className="py-20 sm:py-24 bg-slate-50 border-b border-slate-200/70">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Status bar with Daily Edition indicator */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 p-3 rounded-xl bg-white border border-slate-200/80 shadow-xs">
          <div className="flex flex-wrap items-center gap-3 text-sm text-slate-600">
            <div className="flex items-center gap-2">
              ...
              <span>
                Topic: <strong className="font-semibold text-slate-900">{data?.topic || topic}</strong>
              </span>
            </div>

            <span className="hidden sm:inline text-slate-300">•</span>

            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-50 border border-emerald-200 text-xs font-medium text-emerald-800">
              <Calendar className="w-3 h-3 text-emerald-600" />
              <span>{data?.dailyEdition || 'Today\'s Daily Edition'}</span>
            </div>

            <div className="hidden md:inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-blue-50 border border-blue-200 text-xs font-medium text-blue-700">
              <CheckCircle2 className="w-3 h-3 text-blue-500" />
              <span>Updated Daily (Once a Day)</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto">
            <button
              type="button"
              onClick={() => mutate(() => fetchNews(topic, true))}
              disabled={isValidating}
              title="Run daily refresh"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 disabled:opacity-50 transition-colors"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${isValidating ? 'animate-spin' : ''}`} />
              {isValidating ? 'Checking...' : 'Refresh Daily'}
            </button>
          </div>
        </div>
        ...
        {error && !isLoading ? (
          ...
          <>
            ...
            <div className="mt-6 pt-4 border-t border-slate-200/60 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs text-slate-500">
              <p>
                <strong>Daily Edition:</strong> {data.dailyEdition || new Date(data.generatedAt).toLocaleDateString()} · Updated once a day · Powered by Groq AI and Google News sources.
              </p>
              {data.nextDailyUpdate && (
                <span className="text-slate-400">
                  Next edition refresh: {new Date(data.nextDailyUpdate).toLocaleDateString()}
                </span>
              )}
            </div>
          </>
